...
public class PFPModel extends AbstractModel {
		...
		//插入新节点的边
			...
			if (randomAction <= p + q) { //以概率q
				...
				for (int j = 0; j < 2;) {
					...
					//internalEdge.setNodeB(net,);//根据模型选择邻居节点
					//Debug.outn("PFP model insertNodeWithEdges() new Edge "+prefNode.getIdx()+" "+ internalEdge.getBIdx());

					if (net.addEdge(internalEdge, prefNode, chooseOneNode(net))) { //确保两条边成功建立，不重复。
				...
				for (int j = 0; j < 2;) {
					...
					;
					if (net.addEdge(newEdge, newNode, prefNode)) { //确保两条边成功建立，不重复。
	...
	public boolean preferentialChoosNode(AbstractNetwork net, AbstractV node) {
		...
		if (net.getVertex(node)==null) { //若节点不存在
